...
export function ChatPage({
  ...
  const updateVisibleRange = () => {
    if (scrollableDivRef.current) {
      ...
      const messageHeight = 100; // Estimate average message height
      const bufferCount = 5; // Number of extra messages to render above and below

      const start = Math.max(
        0,
        Math.floor(scrollTop / messageHeight) - bufferCount
      );
      const end = Math.min(
        messageHistory.length,
        Math.ceil((scrollTop + clientHeight) / messageHeight) + bufferCount
      );
  ...
  useEffect(() => {
    ...
    if (scrollableDiv) {
      scrollableDiv.addEventListener("scroll", updateVisibleRange);
      return () =>
        scrollableDiv.removeEventListener("scroll", updateVisibleRange);
    }
  }, []);
